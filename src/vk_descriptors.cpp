...
void DescriptorLayoutBuilder::add_binding(uint32_t bindning, VkDescriptorType type)
{
	VkDescriptorSetLayoutBinding newbind{};
	newbind.binding = bindning;
...
VkDescriptorSetLayout DescriptorLayoutBuilder::build(VkDevice device, VkShaderStageFlags shaderStage, void* pNext, VkDescriptorSetLayoutCreateFlags flags)
{
	for(auto& b : bindings)
	{
...
void DescriptorWriter::write_buffer(int binding, VkBuffer buffer, size_t size, size_t offset, VkDescriptorType type)
{
	...
	VkWriteDescriptorSet write = { .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET };
	write.dstBinding = binding;
	write.dstSet = VK_NULL_HANDLE; // Left null until handled later
	write.descriptorCount = 1;
	write.descriptorType = type;
